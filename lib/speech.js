...
// ── Voice settings ────────────────────────────────────────────────────────────
const EN_SETTINGS = { rate: 1.08, pitch: 1.0, volume: 1.0 }
const HI_SETTINGS = { rate: 1.08, pitch: 1.0, volume: 1.0 }
...
function _clean(text) {
  ...
  return text
    ...
    .replace(/\n{2,}/g, '. ')
    .replace(/\n/g, ', ')
    .replace(/([.!?।])\s*[.,]\s*/g, '$1 ')  // collapse "!." / "?." / ".," left by paragraph-break-to-period above
    .replace(/\s{2,}/g, ' ')
    .trim()
}
...
// One real sentence per chunk (not grouped ~220-char blocks) — each utterance
// boundary is a genuine pause point, so the rhythm reads like natural spoken
// sentences instead of run-on machine-gun paragraphs. Only wrapped by length
// when a single sentence is too long for one utterance.
function _splitIntoSpeechChunks(text, maxLength = 200) {
...
// Small human-like pause between sentences (breathing room), plus a touch of
// randomness so consecutive sentences don't all click over at an identical
// mechanical interval.
const SENTENCE_GAP_MS = 70
function _sentenceGap() {
  return SENTENCE_GAP_MS + Math.round(Math.random() * 50)
}

// ±3% rate jitter per chunk so pacing doesn't sound perfectly uniform —
// real speech never holds one exact rate sentence after sentence.
function _rateJitter() {
...
export function speakText(text, onComplete, options = {}) {
  ...
  const speakNextChunk = () => {
    ...
    const jitter = options.rate ? 1 : _rateJitter()
    utterance.rate = options.rate || _clamp(cfg.rate * mod.rate * jitter, 0.6, 1.3)
    ...
    utterance.onend = () => {
      ...
      if (chunkIndex >= chunks.length) {
        ...
        // Brief natural pause between sentences instead of chaining
        // utterances back-to-back — reads like a person speaking, not a
        // machine firing off lines with zero breath between them.
        setTimeout(() => {
