...
export async function speakText(text, onComplete, options = {}) {
  ...
  if (synthesizerSupported) {
    ...
    try {
      ...
      if (lang) {
        utterance.lang = lang
      } else if (avatarType === 'hindi-teacher') {
      ...
      // Deep male style for Hindi teacher
      ...
      try {
        ...
        if (avatarType === 'hindi-teacher' || (lang && lang.startsWith('hi'))) {
          // Prefer a Hindi male/deep voice if identifiable by name
          preferredVoice = voices.find(voice => voice.lang && voice.lang.toLowerCase().startsWith('hi') && /male|deep|standard[- ]?d|neural2[- ]?d/i.test(voice.name))
      ...
      await useGeminiTTS(text, onComplete, { lang, avatarType })
    }
  } else {
 ...
 * @param {Function} onComplete - Optional callback when speaking is complete
 */
async function useGeminiTTS(text, onComplete, options = {}) {
  const { lang, avatarType } = options
  try {
